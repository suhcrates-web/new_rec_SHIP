...
def carrier_updater():
    ...
    cursor.execute(
        ...
        """,(mat_b.tobytes(), np.array(gid_list).astype('U9').tobytes(), np.array(title_list).astype('U59').tobytes(), np.array(url_list).astype('U68').tobytes(), np.array(thumburl_list).astype('U65').tobytes(), default0.astype('float32').tobytes())
    )
